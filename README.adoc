...
----
include::src/main/java/movies/spring/data/neo4j/domain/Position.java[tags=movie]
...
public interface PositionRepository extends Neo4jRepository<Position, Long> {
    ...
    Position findByTitle(@Param("title") String title);

    Collection<Position> findByTitleLike(@Param("title") String title);

    @Query("MATCH (m:Position)<-[r:ACTED_IN]-(a:Person) RETURN m,r,a LIMIT {limit}")
    Collection<Position> graph(@Param("limit") int limit);
}
----
include::src/main/java/movies/spring/data/neo4j/repositories/PositionRepository.java[tags=repository]

In our case, we use the repository from a `PositionService` to compute the graph representation for the visualization.
...
The other two endpoints for finding multiple movies by title and loading a single movie are provided out of the box by the https://projects.spring.io/spring-data-rest/[Spring-Data-Rest project], which exposes our `PositionRepository` as REST endpoints.
